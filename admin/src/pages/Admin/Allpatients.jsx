    ...
    <div className="w-full max-w-7xl mx-auto px-6 py-8 bg-gray-50">
      <div className="space-y-8">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
          <div>
            <h1 className="text-3xl font-bold text-blue-800">Patient Records</h1>
            <p className="text-blue-600 mt-1 font-medium">Manage patient information</p>
          </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
          ...
          {/* Desktop View Header */}
          <div className="hidden sm:grid grid-cols-[minmax(60px,0.5fr)_3fr_repeat(5,minmax(80px,1fr))] items-center py-4 px-6 bg-blue-50/80 backdrop-blur text-blue-800 text-sm font-semibold uppercase tracking-wide border-b border-blue-100">
            <span>#</span>
            <span>Patient</span>
            <span>Age</span>
            <span>Gender</span>
          ...
          <div className="relative max-h-[70vh] overflow-y-auto">
            {allpatients.map((item, index) => (
              ...
              >
                ...
                >
                  ...
                  <span className="capitalize px-2 py-1 bg-blue-50 text-blue-700 text-xs w-fit font-medium rounded-full">{item.gender}</span>
                  <span className="font-medium text-blue-700">{item.visits}</span>
                  <span className="truncate">Dr. {item.doctorInfo.name}</span>
                  <span className="text-sm font-medium px-2 py-1 bg-teal-50 w-fit text-teal-700 rounded-lg truncate">{item.doctorInfo.speciality}</span>
                </div>
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-center mb-1">
                      ...
                      <span className="ml-2 text-xs text-blue-700 bg-blue-50 px-2 py-0.5 rounded-full capitalize">{item.gender}</span>
                    </div>
                    <div className="flex items-center text-xs text-gray-500">
                      <span className="mr-2">Age: {calculateAge(item.date_of_birth)}</span>
                      <span className="mr-2">•</span>
                      <span className="mr-2">Visits: <span className="text-blue-600 font-medium">{item.visits}</span></span>
                      <span className="mr-2">•</span>
                      <span className="truncate">Dr. {item.doctorInfo.name}</span>
                    </div>
                  </div>
            ...
            {allpatients.length === 0 && (
              <div className="py-12 text-center text-gray-500">
                ...
                <p className="text-lg font-medium text-blue-800">No patients found</p>
                <p className="text-sm text-gray-500 mt-2">Patient records will appear here</p>
              </div>
            )}
          </div>
        </div>
