...
const PatientRecord_Admin = () => {
  const { id } = useParams();
  const { aToken, getPatientRecord, patientRecord, editPatientRecord } = useContext(AdminContext);
  ...
  const handleOverviewSubmit = (e) => {
    e.preventDefault();
    if(formOverviewData.name === '' || formOverviewData.gender === '' || formOverviewData.date_of_birth === '' ){
            ...
            <div className="space-y-6">
              {patientRecord.visits.map((visit, index) => (
                <div key={index} className="space-y-6">
                  <h2 className="text-lg font-medium text-gray-800">Visit {index + 1}</h2>
