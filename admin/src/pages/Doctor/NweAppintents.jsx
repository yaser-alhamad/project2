      ...
      <div className="space-y-8">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            ...
            <p className="text-gray-600 mt-2">Manage upcoming patient sessions</p>
          </div>
          <div className="flex gap-4">
            <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-200">
              <span className="text-gray-600">Total Appointments</span>
              <div className="text-2xl font-bold text-gray-900 mt-1">{newAppointments.length}</div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {newAppointments.map((item, index) => (
            <div key={index} className="bg-white rounded-2xl shadow-sm border border-gray-200 hover:shadow-md transition-all">
              {/* Card Header */}
              ...
              {!item.cancelled ?(
                  ...
                  <div className="text-sm text-gray-600">
                    {item.isCompleted ? 'Session completed' : 'Appointment cancelled'}
                  </div>
                )}
              </div>
              </div>
              ...
              <div className="p-6 border-t border-gray-100 flex items-center justify-end">
                {item.isRecord ? (
                  ...
                  onClick={() => navigate(`/doctor/patientrecord_doctor/${item.recordId}`)}>
                    <FaFileMedical className="w-5 h-5" />
                    <span className="font-medium">Record</span>
                  </button>
                ) : (
                  <button className="flex items-center gap-2 text-blue-600 hover:text-blue-700"
                  onClick={() => navigate(`/doctor/addnewpatientrecord/${item.userData.id}`)}>
