...
const PatientRecord_Doctor = () => {    
  ...
  const [showEditMedicationsModal, setShowEditMedicationsModal] = useState(false);
  ...
  const handleDeleteMedicalHistory = (index) => {
    const updatedData = {
      ...patientRecord,
      medical_history: patientRecord.medical_history.filter((_, i) => i !== index)
    };
    editPatientRecord(id, updatedData);
  };
  ...
  const handleAddImmunization = (e) => {
    ...
    if (!newImmunization.vaccine || !newImmunization.doses || !newImmunization.date) {
  ...
  const handleAddVisit = (e) => {
    e.preventDefault();
    if (!newVisit.date || !newVisit.reason || !newVisit.vital_signs || !newVisit.physician_notes || !newVisit.docName) {
  ...
  return (
    <div className="container mx-auto px-4 sm:px-6 lg:px-8">
      <div className="max-w-4xl mx-auto p-6 space-y-4">
        <div className="bg-gradient-to-br to-indigo-50 rounded-2xl p-8 shadow-lg border border-sky-100">
          <div className="flex items-center gap-3 mb-6">
            ...
            <h2 className="text-2xl font-bold text-sky-900">Patient Overview</h2>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5 text-gray-700">
            ...
            <InfoItem
              label="Date of Birth"
              value={patientRecord?.date_of_birth ? new Date(patientRecord.date_of_birth).toLocaleDateString() : 'N/A'}
            />
            <InfoItem label="Gender" value={patientRecord?.gender || 'N/A'} />
            <InfoItem label="Contact" value={patientRecord?.contact?.phone || 'N/A'} />
            <div className="md:col-span-2">
              <InfoItem
                label="Address"
                value={`${patientRecord?.contact?.address?.line1 || ''}, ${patientRecord?.contact?.address?.line2 || ''}`}
              />
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <SectionCard icon={<DocumentTextIcon className="w-6 h-6 text-green-600" />} title="Medical History">
              ...
              {patientRecord?.medical_history?.length > 0 ? (
                patientRecord.medical_history.map((item, idx) => (
                  ...
                  >
                    ...
                    <button onClick={() => handleDeleteMedicalHistory(idx)} title="Delete Medical History">
            ...
            <div className="flex justify-end mt-4 flex-col gap-2">
              <button onClick={() => setShowAddMedicalHistoryModal(true)} className="flex items-center gap-1 text-sm text-sky-700 bg-white hover:bg-sky-100 border border-sky-200 px-3 py-1 rounded-lg shadow-sm">
          ...
          <SectionCard icon={<ClipboardCheckIcon className="w-6 h-6 text-purple-600" />} title="Active Medications">
            <div className="space-y-3">
              {patientRecord?.medications?.length > 0 ? (
                patientRecord.medications.map((med) => (
                  <div key={med._id} className="flex items-center justify-between border-l-4 border-purple-100 pl-3">
                    <div>
                      <p className="font-medium text-gray-800">{med.name}</p>
                      <p className="text-sm text-gray-500">{med.dosage} • {med.frequency}</p>
                    </div>
                    <button onClick={() => handleDeleteMedication(med._id)} title="Delete Medication">
                      <TrashIcon className="w-5 h-5 text-red-500" />
                    </button>
                  </div>
            ...
            <div className="flex justify-end mt-4 flex-col gap-2">
              <button onClick={() => setShowEditMedicationsModal(true)} className="flex items-center gap-1 text-sm text-sky-700 bg-white hover:bg-sky-100 border border-sky-200 px-3 py-1 rounded-lg shadow-sm">
                <PlusIcon className="w-4 h-4" />
                Add Medication
              </button>
            </div>
          ...
          <SectionCard icon={<ShieldExclamationIcon className="w-6 h-6 text-red-500" />} title="Allergies">
              ...
              {patientRecord?.allergies?.length > 0 ? (
                patientRecord.allergies.map((allergy, idx) => (
                  ...
                  >
                    <div className="flex items-center gap-2">
                      {allergy}
                      <button onClick={() => handleDeleteAllergy(idx)} title="Delete Allergy">
                        <TrashIcon className="w-5 h-5 text-red-500" />
                      </button>
                    </div>
                  </span>
            ...
            <div className="flex justify-end mt-4 flex-col gap-2">
              <button onClick={() => setShowAddAllergyModal(true)} className="flex items-center gap-1 text-sm text-sky-700 bg-white hover:bg-sky-100 border border-sky-200 px-3 py-1 rounded-lg shadow-sm">
                <PlusIcon className="w-4 h-4" />
                Add Allergy
              </button>
            </div>
          ...
          <SectionCard icon={<BeakerIcon className="w-6 h-6 text-blue-500" />} title="Immunizations">
            <ul className="space-y-2">
              {patientRecord?.immunizations?.length > 0 ? (
                patientRecord.immunizations.map((imm, idx) => (
                  <div key={imm._id} className="flex  items-center justify-between border-l-4 border-blue-100 pl-3">
                    <div className="flex flex-col gap-2">
                      <p className="font-medium text-gray-800">{imm.vaccine}</p>
                       <p className="text-sm text-gray-500">
                      Doses: {imm.doses} • Date: {new Date(imm.date).toLocaleDateString()}
                    </p>
                    </div>
                   
                    <button onClick={() => handleDeleteImmunization(idx)} title="Delete Immunization">
                      <TrashIcon className="w-5 h-5 text-red-500" />
                    </button>
                  </div>
                ))
            ...
            <div className="flex justify-end mt-4 flex-col gap-2">
              <button onClick={() => setShowAddImmunizationModal(true)} className="flex items-center gap-1 text-sm text-sky-700 bg-white hover:bg-sky-100 border border-sky-200 px-3 py-1 rounded-lg shadow-sm">
                <PlusIcon className="w-4 h-4" />
                Add Immunization
              </button>
            </div>
        ...
        <SectionCard icon={<CalendarIcon className="w-6 h-6 text-orange-500" />} title="Recent Visit">
          {patientRecord?.visits?.length > 0 ? (
            <div className="space-y-6">
              {patientRecord.visits.map((visit, index) => (
                <div key={index} className="space-y-6">
                  <h2 className="text-lg font-medium text-gray-800 border-b border-gray-200 pb-2 ">Visit {index + 1}</h2>

                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <InfoBox label="Visit Date" value={new Date(visit.date).toLocaleDateString()} />
                    <InfoBox label="Next Appointment" value={new Date(visit.next_appointment).toLocaleDateString()} />
                    <InfoBox label="Reason" value={visit.reason} />
                    <InfoBox label="Blood Pressure" value={visit.vital_signs.blood_pressure} />
                  </div>
                  <InfoBox label="Physician Name" value={'Dr. '+visit.doctor_name} />
                  
                  <div className="bg-blue-50 p-4 rounded-lg">
                    <h4 className="font-medium text-blue-800 mb-2">Physician Notes</h4>
                    <p className="text-blue-700 leading-relaxed">{visit.physician_notes}</p>
                  </div>
                  
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <VitalSign label="Heart Rate" value={`${visit.vital_signs.heart_rate} bpm`} />
                    <VitalSign label="Weight" value={`${visit.vital_signs.weight_lbs} lbs`} />
                    <VitalSign label="Blood Glucose" value={`${visit.vital_signs.blood_glucose_mg_dl} mg/dL`} />
                    <button onClick={() => handleDeleteVisit(index)} title="Delete Visit">
                    <TrashIcon className="w-5 h-5 text-red-500" />
                  </button>
                  </div>
      ...
      {showEditOverviewModal && (
        <div className="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-lg space-y-4">
            <h3 className="text-xl font-semibold text-gray-800">Edit Patient Info</h3>
            <form onSubmit={handleOverviewSubmit} className="space-y-4">
              ...
              <div className="flex justify-end gap-2">
                <button type="button" onClick={() => setShowEditOverviewModal(false)} className="px-4 py-2 bg-gray-200 rounded">
                  Cancel
                </button>
      ...
      {showEditMedicationsModal && (
        <div className="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-lg space-y-4">
            <h3 className="text-xl font-semibold text-gray-800">Add Medications</h3>
            <form onSubmit={handleAddMedication} className="space-y-4">
              ...
              />
              <div className="flex justify-end gap-2">
                <button type="button" onClick={() => setShowEditMedicationsModal(false)} className="px-4 py-2 bg-gray-200 rounded">
                  Cancel
                </button>
                <button type="submit" className="px-4 py-2 bg-sky-600 text-white rounded hover:bg-sky-700">
                  Add
      ...
      {showAddMedicalHistoryModal && (
        <div className="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-lg space-y-4">
            <h3 className="text-xl font-semibold text-gray-800">Add Medical History</h3>
            <form onSubmit={handleAddMedicalHistory} className="space-y-4">
              ...
              />
              <div className="flex justify-end gap-2">
                <button type="button" onClick={() => setShowAddMedicalHistoryModal(false)} className="px-4 py-2 bg-gray-200 rounded">
                  Cancel
                </button>
                <button type="submit" className="px-4 py-2 bg-sky-600 text-white rounded hover:bg-sky-700">
                  Add
        ...
        <div className="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-lg space-y-4">
            ...
            <form onSubmit={handleAddAllergy} className="space-y-4">
              ...
              />
              <div className="flex justify-end gap-2">
                <button type="button" onClick={() => setShowAddAllergyModal(false)} className="px-4 py-2 bg-gray-200 rounded">
                  Cancel
                </button>
                <button type="submit" className="px-4 py-2 bg-sky-600 text-white rounded hover:bg-sky-700">
                  Add
      ...
      {showAddImmunizationModal && (
        <div className="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-lg space-y-4">
            <h3 className="text-xl font-semibold text-gray-800">Add Immunization</h3>
            <form onSubmit={handleAddImmunization} className="space-y-4">
              ...
              />
              <div className="flex justify-end gap-2">
                <button type="button" onClick={() => setShowAddImmunizationModal(false)} className="px-4 py-2 bg-gray-200 rounded">
                  Cancel
                </button>
                <button type="submit" className="px-4 py-2 bg-sky-600 text-white rounded hover:bg-sky-700">
                  Add
        ...
        <div className="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-lg space-y-4">
            ...
            <form onSubmit={handleAddVisit} className="space-y-2">
              <p className="text-gray-800 font-semibold">Date</p>
              <input type="date" name="date" value={newVisit.date} onChange={handleNewVisitChange} placeholder="Date" className="w-full px-4 py-2 border rounded" />
              
              <p className="text-gray-800 font-semibold">Reason</p>
              <input type="text" name="reason" value={newVisit.reason} onChange={handleNewVisitChange} placeholder="Reason" className="w-full px-4 py-2 border rounded" />
              <p className="text-gray-800 font-semibold">Physician Name</p>
              <input type="text" name="doctor_name" value={newVisit.doctor_name} onChange={handleNewVisitChange} placeholder="Physician Name" className="w-full px-4 py-2 border rounded" />
              ...
              <div className="flex flex-shrink gap-2">
                ...
                <input type="text" name="vital_signs"  onChange={handleNewVisitChange} 
                 className="w-full px-2   py-2 border rounded" />
                </span>
                <span className="flex flex-col gap-1 items-center justify-center">
                <p className="text-gray-600 text-sm font-semibold">Heart Rate</p>
                <input type="number" name="vital_signs"  onChange={handleNewVisitChange} 
                 className="w-full px-2  py-2 border rounded" />
                </span>
                <span className="flex flex-col gap-1 items-center justify-center">
                ...
                <span className="flex flex-col gap-1 items-center justify-center  ">
                <p className="text-gray-600 text-sm font-semibold">Blood Glucose</p>
                <input type="number" name="vital_signs"  onChange={handleNewVisitChange} 
                  className="w-full px-2  py-2 border rounded" />
                </span>
              ...
              <textarea type="text" name="physician_notes" rows={5} value={newVisit.physician_notes} onChange={handleNewVisitChange} placeholder="Physician Notes" className="w-full px-2 py-2 border rounded" />
              <p className="text-gray-800 font-semibold">Next Appointment</p>
              <input type="date" name="next_appointment" value={newVisit.next_appointment} onChange={handleNewVisitChange} placeholder="Next Appointment" className="w-full px-4 py-2 border rounded" />
              <div className="flex justify-end gap-2">
                <button type="button" onClick={() => setShowAddVisitModal(false)} className="px-4 py-2 bg-gray-200 rounded">
                  Cancel
                </button>
                <button type="submit" className="px-4 py-2 bg-sky-600 text-white rounded hover:bg-sky-700">
                  Add
