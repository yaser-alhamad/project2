...
const MyProfile = () => {
    ...
    const handleAddpatient = (e) => {
      ...
      const addNewPatient =async(newPatient)=>{
        try{
            const { data } = await axios.post(backendUrl + '/api/user/add-Patient', {newPatient}, { headers: { token } })
            if (data.success) {
            ...
            <div className="bg-white rounded-2xl shadow-lg overflow-hidden mb-8 border border-gray-100 relative">
                ...
                {!isEdit && (
                    <button
                        ...
                        className="absolute top-4 right-4 px-4 py-2 border border-blue-600 text-blue-600 rounded-lg hover:bg-blue-50 transition-colors text-sm font-semibold shadow-sm z-10"
                    >
                        Edit Profile
                    </button>
                )}
                <div className="flex flex-col md:flex-row items-center p-8 gap-6">
                    ...
                    <div className="flex-1 text-center md:text-left">
                        {isEdit ? (
                            <input 
                                className="w-full text-3xl font-bold mb-2 px-3 py-2 bg-gray-50 rounded-lg border border-gray-200 focus:outline-none focus:ring-2 focus:ring-blue-500"
                                type="text" 
                                value={userData.name} 
                ...
                <div className="bg-white rounded-2xl shadow-md p-7 border border-gray-100">
                    ...
                    <div className="space-y-5">
                        <div>
                            ...
                            {isEdit ? (
                                <input 
                                    className="w-full px-3 py-2 bg-gray-50 rounded-lg border border-gray-200 focus:outline-none focus:ring-2 focus:ring-blue-500"
                                    type="text" 
                                    value={userData.phone} 
                                    onChange={(e) => setUserData(prev => ({ ...prev, phone: e.target.value }))}
                                />
                            ) : (
                                <p className="text-gray-800 text-base">{userData.phone || 'Not provided'}</p>
                            )}
                        </div>
                        <div>
                    ...
                    <div className="space-y-5">
                        <div>
                            ...
                            {isEdit ? (
                                ...
                                <p className="text-gray-800 text-base">{userData.gender || 'Not specified'}</p>
                            )}
                        </div>
                        <div>
                            ...
                            {isEdit ? (
                                ...
                                <p className="text-gray-800 text-base">{userData.dob || 'Not specified'}</p>
                            )}
                        </div>
                    </div>
                </div>
            ...
            {isEdit && (
                <div className="mt-10 flex justify-end space-x-4">
                    ...
                    <button 
                        ...
                        className="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 shadow"
                    >
                        Save Changes
                    </button>
                </div>
                ...
                <div className="bg-white rounded-2xl shadow-md p-7 mt-8 border border-gray-100">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {userData.patients.map((patient) => (
                            <div key={patient._id} className="bg-gray-50 rounded-lg p-4 border border-gray-100 shadow-sm relative">
                               
                                <h3 className="font-medium text-gray-800 text-lg">{patient.name}</h3>
                ...
                <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
                    <div className="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-md space-y-6 border border-gray-200 relative">
                        ...
                        <form onSubmit={handleAddpatient} className="space-y-4">
                            ...
                            <div>
                                <label className="block text-gray-700 font-medium mb-1">Patient dob </label>
                                <input
                                    type="date"
                                    name="dob"
